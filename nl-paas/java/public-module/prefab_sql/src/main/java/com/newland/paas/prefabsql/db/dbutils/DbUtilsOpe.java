...
import com.newland.paas.sbcommon.common.PageParamsUtil;
...
public class DbUtilsOpe {
	...
	public static PageBean queryFilter( Connection con, String sqlId, String sql, Object[] param, PageParams pageParams,
        ...
        pageBean.setTotalRecord(totalObject.longValue());
		...
		if (dataMeta.getDriverName().indexOf("JDBC-ODBC Bridges") > -1) {
			...
			indexs[0] = PageParamsUtil.getStartIndex(pageParams);
			indexs[1] = PageParamsUtil.getEndIndex(pageParams);
			...
			newSqlTemp.append(" limit ").append(pageParams.getPageSize());
...
//			newSqlTemp.append(" limit ").append(PageParamsUtil.getStartIndex(pageParams) -1);
			newSqlTemp.append(" , ").append(pageParams.getPageSize());
			newSql =  newSqlTemp.toString();
			...
			System.arraycopy(param, 0, indexs, 0, param.length);
			indexs[indexs.length - 1] = PageParamsUtil.getStartIndex(pageParams);
			indexs[indexs.length - 2] = PageParamsUtil.getEndIndex(pageParams);
		}
			
		MapListHandler rsh = new MapListHandler();
		result  = DbUtilsOpe.query(con, sqlId, newSql, rsh, indexs, prefabSqlStatement);

		pageBean.setCurrentPage(pageParams.getCurrentPage());
		pageBean.setPageSize(pageParams.getPageSize());
