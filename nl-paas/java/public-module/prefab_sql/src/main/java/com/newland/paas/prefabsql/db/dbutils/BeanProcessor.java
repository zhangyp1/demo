...
import com.newland.paas.prefabsql.util.ExtString;
...
public class BeanProcessor {
	//private static LogObj logCache = LogObjFactory.getLogObj(BeanProcessor.class);
	...
	private <T> T createBean(ResultSet rs, Class<T> type, PropertyDescriptor[] props, int[] columnToProperty)
		...
		for (int i = 1; i < columnToProperty.length; i++) {
			...
			try {
				...
				//logCache.error(Constant.LOG_CSF_DBSQL, "createBean errors:" + "给字段：" + prop.getName()
					//	+ "赋值失败，请比较bean和数据库字段的类型是否一致，" + e.getMessage()+"。");
				throw new SQLException("createBean errors:" + "给字段：" + prop.getName()
						+ "赋值失败，请比较bean和数据库字段的类型是否一致，" + e.getMessage()+"。");
			}
		}
	...
	private void callSetter(Object target, PropertyDescriptor prop, Object value) throws SQLException {
		...
		try {
			...
			if (value instanceof java.util.Date) {
				...
				else if("java.util.Date".equals(targetType)){
					value = org.apache.commons.lang.time.DateUtils.truncate(value, Calendar.SECOND);
				} 
			} else if (value instanceof java.sql.Blob) {
				// System.out.println("callSetter value="+ value);
				final String targetType = params[0].getName();
				 ...
				 * else if ("java.sql.Blob".equals(targetType)){
				 * 
				 * }
				 */
				...
				if ("java.lang.String".equals(targetType)) {
					value = sClob;
				} else if ("java.lang.StringBuffer".equals(targetType)) {
					value = new StringBuffer(sClob);
				} else {
					value = "";
				}
			...
			if (this.isCompatibleType(value, params[0])) {
				...
				//logCache.error(Constant.LOG_CSF_DBSQL, "Cannot set " + prop.getName()
					//	+ ": incompatible types, cannot convert " + value.getClass().getName() + " to "
						//+ params[0].getName());
			...
			//logCache.error(Constant.LOG_CSF_DBSQL, "callSetter errors:" + "Cannot set " + prop.getName()
				//	+ ": " + e.getMessage());
	...
	protected <T> T newInstance(Class<T> c) throws SQLException {
		try {
			...
			//logCache.error(Constant.LOG_CSF_DBSQL, "Cannot create " + c.getName() + ": " + e.getMessage());
	...
	private PropertyDescriptor[] propertyDescriptors(Class<?> c) throws SQLException {
		...
		try {
			...
			//logCache.error(Constant.LOG_CSF_DBSQL, "Bean introspection failed: " + e.getMessage());
	...
	protected int[] mapColumnsToProperties(ResultSetMetaData rsmd, PropertyDescriptor[] props)
		...
		for (int col = 1; col <= cols; col++) {
			...
			// add 2015-5 数据库表字段带下划线，而Bean识别是根据方法（驼峰标识的方法）来识别，因此转换来匹配
			propertyName = ExtString.toFirstLetterUpperCase(propertyName);
			for (int i = 0; i < props.length; i++) {
				//if (propertyName.equalsIgnoreCase(props[i].getName())) {
				if (propertyName.equalsIgnoreCase(props[i].getName()) || oldPropertyName.equalsIgnoreCase(props[i].getName())) {
