 ...
 * 
 * Modify By Sun Ying. Fix the problem that the original regular expression can
 * not match "." . This problem can also cause FTPClient.listFiles() return
 * null.
 * 
...
public class UnixFTPEntryParser extends ConfigurableFTPFileEntryParserImpl {
	...
	public static final FTPClientConfig NUMERIC_DATE_CONFIG = new FTPClientConfig(FTPClientConfig.SYST_UNIX,
			NUMERIC_DATE_FORMAT, null, null, null, null);
	...
	private static final String REGEX = "([bcdlfmpSs-])"
			...
			+ "(((r|-)(w|-)([xsStTL-]))((r|-)(w|-)([xsStTL-]))((r|-)(w|-)([xsStTL-])))\\+?\\.*\\s+" + "(\\d+)\\s+"
			+ "(\\S+)\\s+" + "(?:(\\S+)\\s+)?" + "(\\d+)\\s+"
