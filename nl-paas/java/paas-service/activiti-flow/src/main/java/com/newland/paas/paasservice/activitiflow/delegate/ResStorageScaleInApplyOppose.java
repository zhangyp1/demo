 ...
 * 存储资源缩容申请状态成功修改为：不同意
 */
public class ResStorageScaleInApplyOppose implements JavaDelegate {

    private static final Log LOG = LogFactory.getLogger(CreateTenantApproveDelegate.class);

	@Override
	public void execute(DelegateExecution execution) {
		LOG.info(LogProperty.LOGTYPE_DETAIL, "存储资源缩容申请状态成功修改为：不同意!");
		LOG.info(LogProperty.LOGTYPE_DETAIL, "===ResStorageScaleInApplyOppose.execution===begin");
		...
		String responseContent = restTemplateUtils.
				postForTokenEntity(microservicesProperties.getResmgr(),"v1/storageRes/scaleInOpposeApply",
		...
		LOG.info(LogProperty.LOGTYPE_DETAIL, "===ResStorageScaleInApplyOppose.execution===end");
