 ...
 * @since 2019/7/4
 */
public class BindClusterVolApplyOpposeDelegate implements JavaDelegate {

    private static final Log LOG = LogFactory.getLogger(BindClusterVolApplyOpposeDelegate.class);

    @Override
    public void execute(DelegateExecution execution) {
        LOG.info(LogProperty.LOGTYPE_DETAIL, "===BindClusterVolApplyOpposeDelegate.execution===begin");
        ...
        String responseContent = restTemplateUtils.postForTokenEntity(microservicesProperties.getClumgr(),
                "v1/vol/bind/rollback", ActTokenUtil.getPaasToken(), requestVo, responseType);
        LOG.info(LogProperty.LOGTYPE_DETAIL, "===responseContent:===" + responseContent);
        execution.setVariable("bindRollbackResult", responseContent);
        LOG.info(LogProperty.LOGTYPE_DETAIL, "===BindClusterVolApplyOpposeDelegate.execution===end");
    }
