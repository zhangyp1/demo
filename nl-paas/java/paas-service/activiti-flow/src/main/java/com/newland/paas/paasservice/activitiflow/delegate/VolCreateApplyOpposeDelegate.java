 ...
 * @since 2019/7/4
 */
public class VolCreateApplyOpposeDelegate implements JavaDelegate {

    private static final Log LOG = LogFactory.getLogger(VolCreateApplyOpposeDelegate.class);

    @Override
    public void execute(DelegateExecution execution) {
        LOG.info(LogProperty.LOGTYPE_DETAIL, "===VolCreateApplyOpposeDelegate.execution===begin");
        ...
        String responseContent = restTemplateUtils.postForTokenEntity(microservicesProperties.getClumgr(),
                "v1/vol/createVol/rollback", ActTokenUtil.getPaasToken(), requestVo, responseType);
        LOG.info(LogProperty.LOGTYPE_DETAIL, "===responseContent:===" + responseContent);
        execution.setVariable("relatedCluZone", responseContent);

        LOG.info(LogProperty.LOGTYPE_DETAIL, "===VolCreateApplyOpposeDelegate.execution===end");
    }
