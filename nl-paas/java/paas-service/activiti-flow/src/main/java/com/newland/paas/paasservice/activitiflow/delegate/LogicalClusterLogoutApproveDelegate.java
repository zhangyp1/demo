 ...
 * @since 2019/7/24
 */
public class LogicalClusterLogoutApproveDelegate implements JavaDelegate {

    private static final Log LOG = LogFactory.getLogger(LogicalClusterLogoutApproveDelegate.class);

    @Override
    public void execute(DelegateExecution execution) {
        LOG.info(LogProperty.LOGTYPE_DETAIL, "===LogicalClusterLogoutApproveDelegate.execution===begin");
        ...
        String responseContent = restTemplateUtils.postForTokenEntity(microservicesProperties.getClumgr(),
                "v1/logic/logoutCluster", ActivitiTokenUtils.getPaasToken(), requestParam, responseType);
        LOG.info(LogProperty.LOGTYPE_DETAIL, "===responseContent:===" + responseContent);
        execution.setVariable("result", responseContent);
        LOG.info(LogProperty.LOGTYPE_DETAIL, "===LogicalClusterLogoutApproveDelegate.execution===end");
    }
