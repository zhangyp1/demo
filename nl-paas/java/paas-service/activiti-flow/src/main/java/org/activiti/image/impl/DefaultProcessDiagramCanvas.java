...
public class DefaultProcessDiagramCanvas {
    ...
    protected String activityFontName = "宋体";//Arial
    protected String labelFontName = "宋体";//Arial
    protected String annotationFontName = "宋体";//Arial 宋体 en_US.UTF-8
...
//        }
    ...
    public void drawStartEvent(GraphicInfo graphicInfo, BufferedImage image, double scaleFactor) {
        ...
        Double circle = new Double(graphicInfo.getX(), graphicInfo.getY(), graphicInfo.getWidth(), graphicInfo.getHeight());
        this.g.fill(circle);
    ...
    public void drawCatchingEvent(GraphicInfo graphicInfo, boolean isInterrupting, BufferedImage image, String eventType, double scaleFactor) {
        Double outerCircle = new Double(graphicInfo.getX(), graphicInfo.getY(), graphicInfo.getWidth(), graphicInfo.getHeight());
        ...
        Double innerCircle = new Double((double) innerCircleX, (double) innerCircleY, (double) innerCircleWidth, (double) innerCircleHeight);
        Paint originalPaint = this.g.getPaint();
        Stroke originalStroke = this.g.getStroke();
        this.g.setPaint(EVENT_COLOR);
    ...
    public void drawConnection(int[] xPoints, int[] yPoints, boolean conditional, boolean isDefault, String connectionType, AssociationDirection associationDirection, boolean highLighted, double scaleFactor) {
        ...
        for (int line = 1; line < xPoints.length; ++line) {
            Integer sourceX = Integer.valueOf(xPoints[line - 1]);
            Integer sourceY = Integer.valueOf(yPoints[line - 1]);
            Integer targetX = Integer.valueOf(xPoints[line]);
            Integer targetY = Integer.valueOf(yPoints[line]);
            java.awt.geom.Line2D.Double line1 = new java.awt.geom.Line2D.Double((double) sourceX.intValue(), (double) sourceY.intValue(), (double) targetX.intValue(), (double) targetY.intValue());
            this.g.draw(line1);
        }

        java.awt.geom.Line2D.Double var18;
        if (isDefault) {
            var18 = new java.awt.geom.Line2D.Double((double) xPoints[0], (double) yPoints[0], (double) xPoints[1], (double) yPoints[1]);
            this.drawDefaultSequenceFlowIndicator(var18, scaleFactor);
        }

        if (conditional) {
            var18 = new java.awt.geom.Line2D.Double((double) xPoints[0], (double) yPoints[0], (double) xPoints[1], (double) yPoints[1]);
            this.drawConditionalSequenceFlowIndicator(var18, scaleFactor);
        }

        if (associationDirection.equals(AssociationDirection.ONE) || associationDirection.equals(AssociationDirection.BOTH)) {
            var18 = new java.awt.geom.Line2D.Double((double) xPoints[xPoints.length - 2], (double) yPoints[xPoints.length - 2], (double) xPoints[xPoints.length - 1], (double) yPoints[xPoints.length - 1]);
            this.drawArrowHead(var18, scaleFactor);
        }

        if (associationDirection.equals(AssociationDirection.BOTH)) {
            var18 = new java.awt.geom.Line2D.Double((double) xPoints[1], (double) yPoints[1], (double) xPoints[0], (double) yPoints[0]);
            this.drawArrowHead(var18, scaleFactor);
        }
    ...
    protected void drawTask(String name, GraphicInfo graphicInfo, boolean thickBorder) {
        ...
        java.awt.geom.RoundRectangle2D.Double rect = new java.awt.geom.RoundRectangle2D.Double((double) x, (double) y, (double) width, (double) height, (double) arcR, (double) arcR);
        this.g.fill(rect);
    ...
    public void drawExpandedSubProcess(String name, GraphicInfo graphicInfo, Boolean isTriggeredByEvent, double scaleFactor) {
        java.awt.geom.RoundRectangle2D.Double rect = new java.awt.geom.RoundRectangle2D.Double(graphicInfo.getX(), graphicInfo.getY(), graphicInfo.getWidth(), graphicInfo.getHeight(), 8.0D, 8.0D);
        if (isTriggeredByEvent.booleanValue()) {
            Stroke text = this.g.getStroke();
            this.g.setStroke(EVENT_SUBPROCESS_STROKE);
            this.g.draw(rect);
            this.g.setStroke(text);
        } else {
            Paint text1 = this.g.getPaint();
            this.g.setPaint(SUBPROCESS_BOX_COLOR);
            this.g.fill(rect);
            this.g.setPaint(SUBPROCESS_BORDER_COLOR);
            this.g.draw(rect);
            this.g.setPaint(text1);
        }

        if (scaleFactor == 1.0D && name != null && !name.isEmpty()) {
            String text2 = this.fitTextToWidth(name, (int) graphicInfo.getWidth());
            this.g.drawString(text2, (int) graphicInfo.getX() + 10, (int) graphicInfo.getY() + 15);
        }
    ...
    public void drawHighLight(int x, int y, int width, int height) {
        ...
        this.g.setPaint(HIGHLIGHT_COLOR);
        this.g.setStroke(THICK_TASK_BORDER_STROKE);
        java.awt.geom.RoundRectangle2D.Double rect = new java.awt.geom.RoundRectangle2D.Double((double) x, (double) y, (double) width, (double) height, 20.0D, 20.0D);
        this.g.draw(rect);
        this.g.setPaint(originalPaint);
        this.g.setStroke(originalStroke);
    }
